...
class CalculateRatings implements ShouldQueue
{
    ...
    const MIN_TOTAL_PARTICIPATORS = 5;
    const MIN_TOP1_TIME = 500;
    const MIN_TOP_RELTIME = 0.6;
    const MIN_TOTAL_RECORDS = 50;
    const BANNED_MAPS = ['map1', 'map2', 'map3']; // fill this later
    ...
    const CFG_R = 100; // mid point for logistic decay and also number of best records to focus on
    ...
    public function handle(): void
    {
        ...
        $query = $this->addTotalParticipators($query);
        ...
        $upsert_data = array_map(function ($item){
            return (array) $item;
        }, $result);

        PlayerRating::upsert(
            $upsert_data,
            uniqueBy: ['mdd_id', 'physics', 'mode'],
            update: ['name', 'player_rating']
        );
    ...
    protected function addTotalParticipators($query)
    {
        return DB::table(DB::raw("({$query->toSql()}) as sub"))
            ->mergeBindings($query)
            ->addSelect('*')
    ...
    protected function addBannedMaps($query)
    {
        ...
        return DB::table(DB::raw("({$query->toSql()}) as sub"))
            ...
            ->addSelect(DB::raw('
                CASE
                WHEN total_participators < ' . self::MIN_TOTAL_PARTICIPATORS . '
